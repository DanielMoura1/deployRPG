...
export default function Sel(props){
    ...
    useEffect(() => {
        async function getpg1(){
         try{
            const promessa=await axios.get('https://localhost:5000/api/selecao',{headers: {
                authorization : token
             }})
    ...
    async function b(){
        try {
            if(num===10){
            const resposta = await axios.post(`https://localhost:5000/api/adicionar`,cards,{headers: {
                authorization : token
             }})
