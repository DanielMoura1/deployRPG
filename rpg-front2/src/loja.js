...
export default function Loja(props){
    ...
    useEffect(() => {
        async function getpg1(){
         try{
            //mudar
            const promessa=await axios.get('http://54.158.210.134:5000/api/loja',{headers: {
                authorization : token
             }})
    ...
    async function b(){
        if( cards[0].ouroCard[0].ouro<=perfil[0].ouro ){
            try {
                    //mudar
                const resposta = await axios.post(`http://54.158.210.134:5000/api/loja`,cards,{headers: {
                    authorization : token
                }})
